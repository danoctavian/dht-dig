...
testFold = DF.foldr (:) [] $ Node (Leaf "2" 2) (Leaf "1" 1)
{-   
instance Traversable BitTrie where
   traverse f Empty = pure Empty
   traverse f (Leaf k v) = Leaf <$> f v
...
goUp z@(BTZipper f (c : cs) d) =
  BTZipper ((if' (isLeft z) P.id P.flip Node) f (crumbT c)) cs (d - 1)
-- WARNING: sexist code ahead
brother z = (goDown $ if' (isLeft z) R L) . goUp $ z
rezip = focus . rz where rz z = if' (isTop z) z (rz $ goUp z)

isTop = (== []) . trail 
isLeft zipper = case P.head $ trail zipper of
  LeftCrumb {..} -> True
  RightCrumb {..} -> False  
...
testBadTree = closest "\1\1" 2 badTree
badTree = Node (Node (Leaf "\1\0" 1) (Node (Leaf "\23\24" 25) (Leaf "\29\28" 28)) ) inf
